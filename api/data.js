...
export default async function handler(req, res) {
  try {
    ...
    // Filtrar apenas perpétuos e limitar para desempenho
    const perpMarkets = markets
      .filter(m => m.symbol.endsWith("_PERP"))
      .slice(0, 15);

    const result = await Promise.all(
      perpMarkets.map(async m => {
        try {
          ...
          if (!resp.ok) {
            ...
            console.error(`Erro candles ${m.symbol}:`, text);
            throw new Error(`Falha candles ${m.symbol}`);
          }

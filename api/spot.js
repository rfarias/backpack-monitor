...
const computeATR=(h,l,c,p=14)=>{if(c.length<2)return 0;const t=[];for(let i=1;i<h.length;i++){const tr=Math.max(h[i]-l[i],Math.abs(h[i]-c[i-1]),Math.abs(l[i]-c[i-1]));t.push(tr);}return mean(t.slice(-p));};
const computeRSI=(c,p=9)=>{if(c.length<=p)return 50;const d=c.slice(1).map((x,i)=>x-c[i]);const g=d.map(v=>v>0?v:0);const L=d.map(v=>v<0?Math.abs(v):0);const aG=mean(g.slice(-p));const aL=mean(L.slice(-p))||1e-9;const rs=aG/aL;return 100-100/(1+rs);};
const computeBollinger=(c,p=20,mult=2)=>{if(!c.length)return{middle:0,width:0};const s=c.slice(-p);const m=mean(s);const sd=std(s);return{middle:m,width:(2*mult*sd)/(m||1)};};
const computeEMA=(c,p=20)=>{if(c.length<p)return mean(c);const k=2/(p+1);let ema=c[0];for(let i=1;i<c.length;i++)ema=c[i]*k+ema*(1-k);return ema;};
...
const fetchKlines=async(symbol,interval="15m",limit=200)=>{
  const sec=interval.endsWith("m")?+interval.replace("m","")*60:interval.endsWith("h")?+interval.replace("h","")*3600:1800;
  try{
    ...
    if(r.ok){const d=await r.json();if(Array.isArray(d)&&d.length>0)return d.map(c=>({open:+c.open,high:+c.high,low:+c.low,close:+c.close,volume:+c.volume,ts:+c.openTime}));}
...
export default async function handler(req,res){
  try{
    ...
    for(const m of spot){
      const run=async()=>{
        let lastPrice=0,volumeUSD=0,atrRel=0,rsi=0,bbWidth=0,ema20=0,decision="aguardando",score=0,spreadPct=0,marketCapUSD=0,liquidityScore=0;
        try{
          const tick=await fetch(`${BASE_URL}/ticker?symbol=${m.symbol}`);
          const t=tick.ok?await tick.json():{};
          lastPrice=+t.lastPrice||0;const v=+t.volume||0;volumeUSD=v*lastPrice;
          const kl=await fetchKlines(m.symbol,tf,200);
          if(kl.length>0&&lastPrice>0){
            const c=kl.map(k=>k.close),h=kl.map(k=>k.high),l=kl.map(k=>k.low);
            const atr=computeATR(h,l,c);atrRel=lastPrice?atr/lastPrice:0;
            rsi=computeRSI(c);bbWidth=computeBollinger(c).width;ema20=computeEMA(c);
        ...
        results.push({symbol:m.symbol,lastPrice,atrRel,rsi,bbWidth,ema20,volumeUSD,marketCapUSD,liquidityScore,spreadPct,decision,score,visible:m.visible,orderBookState:m.orderBookState});
      };
      active.push(run());
